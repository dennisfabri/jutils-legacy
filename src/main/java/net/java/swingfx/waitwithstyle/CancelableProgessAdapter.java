 ...
 */
class CancelableProgessAdapter implements InfiniteProgressAdapter {
    ...
    void doPropertyChange(PropertyChangeEvent evt) {
        if ("defaultButton".equals(evt.getPropertyName())) {
            /*
             * Track the default button every time it changes so that when the
             * cancel button becomes the default button, as it must when the
             * user clicks it, its possible to reset the default button back to
             * being the button that the application expects it to be...
             * Ideally, the cancel button should never even get focus as a focus
             * change is not transparent to application code. A better scheme
             * might be to set cancel button to be not focusable and then click
             * the button programmatically as appropriate.
             */
