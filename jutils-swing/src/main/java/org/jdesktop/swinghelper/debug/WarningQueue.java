    ...
    private static void check(long id) {
        ...
        for (MonitorInfo lock : thread.getLockedMonitors()) {
            if (!isInvokeAndWait(lock.getLockedStackFrame())) {
                buff.append("  monitor: ").append(lock.getClassName()).append("@")
                        .append(Integer.toHexString(lock.getIdentityHashCode())).append(" at ")
                        .append(lock.getLockedStackFrame()).append(" [")
                        .append(lock.getLockedStackDepth() - IGNORED_FRAMES).append("]\n");
            }
        }
        for (LockInfo lock : thread.getLockedSynchronizers()) {
            buff.append("  lock:").append(lock.getClassName()).append("@")
                    .append(Integer.toHexString(lock.getIdentityHashCode())).append("\n");
        }
        if (buff.length() != 0) {
