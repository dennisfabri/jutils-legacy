...
public final class AutoCompleteDecorator {
    ...
    private static void installMap(InputMap componentMap, boolean strict) {
        ...
        if (strict) {
            ...
            map.put(KeyStroke.getKeyStroke(java.awt.event.KeyEvent.VK_X, java.awt.event.InputEvent.CTRL_DOWN_MASK),
                    errorFeedbackAction);
        } else {
            // VK_BACKSPACE will move the selection to the left if the selected item is in
            // the list
    ...
    static AutoCompleteDocument createAutoCompleteDocument(AbstractAutoCompleteAdaptor adaptor, boolean strictMatching,
     ...
     * completion will be strict (only items from the combo box can be selected) if
     ...
     * the combo box is not editable.
     * <p>
     * <b>Note:</b> the {@code AutoCompleteDecorator} will alter the state of the
     * {@code JComboBox} to be editable. This can cause side effects with layouts
     * and sizing. {@code JComboBox} caches the size, which differs depending on the
     * component's editability. Therefore, if the component's size is accessed prior
     * to being decorated and then the cached size is forced to be recalculated, the
     ...
     * {@code JComboBox}. Users that need to ensure sizes of supplied combos should
     * take measures to set the size of the combo.
     * 
     * @param comboBox        a combo box
    ...
    private static <T> void decorate(JComboBox<T> comboBox, ObjectToStringConverter stringConverter) {
        ...
        final AbstractAutoCompleteAdaptor adaptor = new ComboBoxAdaptor<T>(comboBox);
        final AutoCompleteDocument document = createAutoCompleteDocument(adaptor, strictMatching, stringConverter,
                editorComponent.getDocument());
    ...
    private static <T> void decorate(JList<T> list, JTextComponent textComponent,
        ...
        AbstractAutoCompleteAdaptor adaptor = new ListAdaptor<T>(list, textComponent, stringConverter);
        AutoCompleteDocument document = createAutoCompleteDocument(adaptor, true, stringConverter,
     ...
     * @param textComponent   the text component that will be used for automatic
     *                        completion.
     * @param strictMatching  <tt>true</tt>, if only given items should be allowed
     *                        to be entered
     * @param stringConverter the converter used to transform items to strings
     */
